# Incident Io 3 TypeScript API Library
...
This library provides convenient access to the Incident Io 3 REST API from server-side TypeScript or JavaScript.
...
Use the Incident Io 3 MCP Server to enable AI assistants to interact with this API, allowing them to explore endpoints, make test requests, and use documentation to help integrate this SDK into your application.
...
const actions: IncidentIo3.ActionListResponse = await client.actions.list();
```
...
const actions = await client.actions.list().catch(async (err) => {
  if (err instanceof IncidentIo3.APIError) {
...
1. Via the `INCIDENT_IO_3_LOG` environment variable
...
const client = new IncidentIo3({
  logger: logger.child({ name: 'IncidentIo3' }),
...
const client = new IncidentIo3({ fetch });
```
...
```ts
import IncidentIo3 from 'npm:incident-io-2';
