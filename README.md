# Incident Io 6 TypeScript API Library
...
This library provides convenient access to the Incident Io 6 REST API from server-side TypeScript or JavaScript.
...
Use the Incident Io 6 MCP Server to enable AI assistants to interact with this API, allowing them to explore endpoints, make test requests, and use documentation to help integrate this SDK into your application.
...
const actions: IncidentIo6.ActionListResponse = await client.actions.list();
```
...
const actions = await client.actions.list().catch(async (err) => {
  if (err instanceof IncidentIo6.APIError) {
...
1. Via the `INCIDENT_IO_6_LOG` environment variable
...
const client = new IncidentIo6({
  logger: logger.child({ name: 'IncidentIo6' }),
...
const client = new IncidentIo6({ fetch });
```
...
```ts
import IncidentIo6 from 'npm:incident-io-2';
