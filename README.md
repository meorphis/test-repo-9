# Incident Io 4 TypeScript API Library
...
This library provides convenient access to the Incident Io 4 REST API from server-side TypeScript or JavaScript.
...
Use the Incident Io 4 MCP Server to enable AI assistants to interact with this API, allowing them to explore endpoints, make test requests, and use documentation to help integrate this SDK into your application.
...
const actions: IncidentIo4.ActionListResponse = await client.actions.list();
```
...
const actions = await client.actions.list().catch(async (err) => {
  if (err instanceof IncidentIo4.APIError) {
...
1. Via the `INCIDENT_IO_4_LOG` environment variable
...
const client = new IncidentIo4({
  logger: logger.child({ name: 'IncidentIo4' }),
...
const client = new IncidentIo4({ fetch });
```
...
```ts
import IncidentIo4 from 'npm:incident-io-2';
