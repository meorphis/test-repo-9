...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['id', 'description', 'instructions', 'name', 'shortform'],
  },
  annotations: {
    idempotentHint: true,
  },
...
export const handler = async (client: IncidentIo2, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.incidentRoles.update(id, body)));
  } catch (error) {
    if (isJqError(error)) {
