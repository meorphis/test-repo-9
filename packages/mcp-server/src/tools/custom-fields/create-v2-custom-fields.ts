...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['description', 'field_type', 'name'],
    $defs: {
      custom_field_filter_by_options_v2: {
        type: 'object',
        properties: {
        ...
        required: ['catalog_attribute_id', 'custom_field_id'],
      },
    },
  },
  annotations: {},
...
export const handler = async (client: IncidentIo2, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.customFields.createV2(body)));
  } catch (error) {
