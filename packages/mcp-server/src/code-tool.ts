...
const prompt = `Runs JavaScript code to interact with the Incident Io 13 API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ...
    INCIDENT_IO_13_BASE_URL: readEnv('INCIDENT_IO_13_BASE_URL') ?? client.baseURL ?? undefined,
  };
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = 'incident-io-2';
