...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? '@stainless-temp/meorphis-test/'
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from '@stainless-temp/meorphis-test/_shims/index'`,
